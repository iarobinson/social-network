...
router.get('/', passport.authenticate('jwt', { session: false }), (req, res) => {
	...
	// Find the user related to _id (req.user.id)
	Profile.findOne({ user: req.user.id })
		.populate('user', ['name', 'avatar'])
		.then(profile => {
			if (!profile) {
				errors.noprofile = "There is no profile from this user";
				return res.status(404).json(errors)
			}

			res.json(profile);
		})
		.catch(err => res.stats(404).json(err));
});
...
router.post('/', passport.authenticate('jwt', { session: false }),
	(req, res) => {
		...
		Profile.findOne( { user: req.user.id })
			.then(profile => {
				if (profile) {
					// The profile exists so the user is trying to update their existing profile
...
router.post('/education', passport.authenticate('jwt', { session: false }), (req, res) => {
	...
	Profile.findOne({ user: req.user.id }) // Find user in DB
		.then((profile) => { // We assign the user data to profile variable
			...
			// Attach the education object to the user data profile
...
router.delete(
	...
	Profile.findOne({ user: req.user.id }) // Find user in DB
		.then((profile) => { // We assign the user data to profile variable
			...
			const removeIndex = profile.experience // Get into experience part of object
				.map(item => item.id) // Get a list of items by the id
				.indexOf(req.params.exp_id); // Find the location of the matching id
